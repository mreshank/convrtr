...
For any note $N \in [1, 84]$, the target frequency $f$ is calculated using equal temperament relative to middle C (note 49 at 8363 Hz):

$$f = 8363 \times 2^{\frac{N - 49}{12}}$$

The per-sample playback increment step is then:

$$\text{step} = \frac{f}{\text{sampleRate}}$$
...
$$\text{sample}(t) = (1 - \text{frac}) \times \text{pcm}[\lfloor t \rfloor] + \text{frac} \times \text{pcm}[\lceil t \rceil]$$
