...
To determine the color of a pixel at row $y$ and column $x$ ($0 \le x, y < 8$):
1. Extract bit $7 - x$ from Bitplane 0 row byte:
   $$\text{bit}_0 = (\text{Plane}_0[y] \gg (7 - x)) \ \& \ 1$$
2. Extract bit $7 - x$ from Bitplane 1 row byte:
   $$\text{bit}_1 = (\text{Plane}_1[y] \gg (7 - x)) \ \& \ 1$$
3. Combine both bits into a 2-bit color index ($0, 1, 2, \text{ or } 3$):
   $$\text{Index} = (\text{bit}_1 \ll 1) \mid \text{bit}_0$$

## Pattern Table Layout and Sizing

Standard NES game cartridges allocate an **8KB CHR bank** (8,192 bytes):
- $8,192 \text{ bytes} \div 16 \text{ bytes/tile} = 512 \text{ tiles}$.
- The PPU divides this into two 4KB pattern tables:
  - Pattern Table 0: Background tiles ($256$ tiles, $128 \times 128$ px).
  - Pattern Table 1: Sprite tiles ($256$ tiles, $128 \times 128$ px).
