...
import type {
	SrfConversionOptions,
	SrfConversionResult,
	SrfMetadata,
} from "./types";

/**
 * Finds the offset and length of an embedded JPEG stream within raw bytes
 * by locating the SOI (0xFF 0xD8 0xFF) and EOI (0xFF 0xD9) markers.
 */
function findEmbeddedJpeg(
	...
	if (eoi === -1 || eoi <= soi) {
		// Use remaining buffer if EOI not found
		eoi = bytes.length;
	}
...
/**
 * Parses Sony Alpha RAW (.srf / .sr2 / .arw) container and extracts
 * the embedded high-resolution preview or raw image to a 32-bit RGBA PNG.
 */
export async function convertSrfToPng(
	input: ArrayBuffer | Uint8Array,
	_options: SrfConversionOptions = {},
	onProgress?: (ratio: number, phase: string) => void,
): Promise<SrfConversionResult> {
	...
	if (!isLE && !isBE) {
		throw new Error(
			"Invalid Sony RAW file: Missing TIFF byte order indicator ('II' or 'MM').",
		);
	}
	...
	if (magic !== 42) {
		throw new Error("Invalid Sony RAW file: Missing TIFF magic 42 constant.");
	}

	onProgress?.(0.3, "PARSE_EXIF");
		...
		for (let i = 0; i < numEntries && entryPos + 12 <= bytes.length; i++) {
			...
			switch (tag) {
				case 0x0100: // ImageWidth
					width = type === 3 ? view.getUint16(entryPos - 4, isLE) : valOrOffset;
					break;
	...
	if (
		...
		// Fallback: search for embedded JPEG stream
	...
	if (jpegBytes && jpegBytes.length > 100) {
		...
		// Fallback: Synthesize placeholder image if no valid preview found
		if (width <= 0 || height <= 0) {
			width = 64;
			height = 64;
		}
	...
	const metadata: SrfMetadata = {
		make: make || "Sony",
		model: model || "Alpha DSLR",
		width,
		height,
		hasEmbeddedPreview,
