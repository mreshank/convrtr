...
export function formatBytes(bytes: number): string {
	...
	// Determine decimals before rounding
	let decimals = value < 10 ? 2 : 1;
	let rounded = Number(value.toFixed(decimals));
	// If rounding promoted us past 1000 and a larger unit exists, move up
	if (rounded >= 1000 && unit < UNITS.length - 1) {
		value = rounded / 1000;
		unit += 1;
		// Recalculate decimals for the new unit
		decimals = value < 10 ? 2 : 1;
		rounded = Number(value.toFixed(decimals));
	}
	// If rounding pushed us to 10+ while decimals was 2, drop to 1
